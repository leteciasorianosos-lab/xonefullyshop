...
<title>Login — GHOSST STORE</title>
...
.blob-1 {
    width: 600px; height: 600px; top: -15%; left: -12%;
    background: radial-gradient(circle at 40% 40%, rgba(57,255,20,.14) 0%, rgba(20,184,184,.09) 35%, transparent 70%);
    animation: blobDrift1 22s ease-in-out infinite alternate;
}
.blob-2 {
    width: 500px; height: 500px; bottom: -10%; right: -10%;
    background: radial-gradient(circle at 55% 55%, rgba(20,184,184,.13) 0%, rgba(57,255,20,.06) 40%, transparent 70%);
    animation: blobDrift2 18s ease-in-out infinite alternate; animation-delay: -6s;
}
.blob-3 {
    width: 350px; height: 350px; top: 40%; right: 15%;
    background: radial-gradient(circle, rgba(0,255,247,.1) 0%, rgba(57,255,20,.05) 45%, transparent 70%);
    animation: blobDrift3 26s ease-in-out infinite alternate; animation-delay: -12s;
}
@keyframes blobDrift1 {
    0%   { transform: translate(0,0)       scale(1)    rotate(0deg); }
    40%  { transform: translate(60px,-50px) scale(1.08) rotate(12deg); }
    100% { transform: translate(-40px,60px) scale(0.94) rotate(-8deg); }
}
@keyframes blobDrift2 {
    0%   { transform: translate(0,0)        scale(1)    rotate(0deg); }
    40%  { transform: translate(-70px,45px) scale(1.06) rotate(-14deg); }
    100% { transform: translate(50px,-60px) scale(0.96) rotate(10deg); }
}
@keyframes blobDrift3 {
    0%   { transform: translate(0,0)       scale(1)    rotate(0deg); }
    50%  { transform: translate(40px,55px) scale(1.1)  rotate(18deg); }
    100% { transform: translate(-50px,-40px) scale(0.92) rotate(-12deg); }
}
...
.login-card {
    ...
    box-shadow:
        0 0 0 1px rgba(20,184,184,.06),
        0 0 50px rgba(20,184,184,.15),
...
.gs-brand {
    ...
    gap: .65rem; text-decoration: none; margin-bottom: 1.75rem;
}
.gs-brand img {
...
.page-title {
    ...
    color: var(--text-hi); text-align: center; margin-bottom: 1.75rem; letter-spacing: .05em;
}
...
.alert-banned  { background: rgba(239,68,68,.12);  border: 1px solid rgba(239,68,68,.35);  color: #fecaca; text-align: center; display: block; padding: 1.5rem; }
.alert-banned i { font-size: 2rem; color: var(--danger); display: block; margin-bottom: .75rem; }
.alert-banned h3 { font-family: 'Orbitron', sans-serif; font-size: 1.1rem; color: #f87171; margin-bottom: .5rem; }
...
.input-icon {
    ...
    color: var(--text-lo); font-size: .85rem; pointer-events: none;
...
.form-input:focus {
    ...
    box-shadow: 0 0 0 3px rgba(20,184,184,.1), var(--glow-sm);
    background: rgba(20,184,184,.07);
}
.form-input:focus + .input-icon,
.input-wrap:focus-within .input-icon { color: var(--teal-2); }

.toggle-pw {
    position: absolute; right: .9rem; top: 50%; transform: translateY(-50%);
    color: var(--text-lo); font-size: .85rem; cursor: pointer;
    border: none; background: none; padding: .25rem; transition: color .2s;
}
.toggle-pw:hover { color: var(--teal-2); }
...
.btn-outline {
    ...
    padding: .75rem 1.5rem; border: 1.5px solid var(--teal-1);
    color: var(--teal-3); background: transparent; border-radius: 10px;
    font-family: 'Orbitron', sans-serif; font-size: .8rem; font-weight: 700;
    letter-spacing: .08em; text-transform: uppercase; text-decoration: none;
    cursor: pointer; transition: all .25s;
}
.btn-outline:hover { background: rgba(20,184,184,.1); border-color: var(--teal-2); box-shadow: var(--glow-sm); }
...
.divider span {
    ...
    letter-spacing: .15em; color: var(--text-lo); text-transform: uppercase;
    white-space: nowrap;
}
...
.card-footer a {
    color: var(--teal-3); font-weight: 600; text-decoration: none;
    font-size: .875rem; transition: color .2s;
}
.card-footer a:hover { color: var(--teal-2); }
.card-footer .muted {
    color: var(--text-lo); font-size: .8rem; text-decoration: none; transition: color .2s;
}
...
.live-dot {
    ...
    background: var(--ecto-green); box-shadow: 0 0 8px var(--ecto-green);
    animation: livepulse 2s infinite; flex-shrink: 0;
}
@keyframes livepulse {
    0%,100%{ box-shadow: 0 0 4px var(--ecto-green); }
    50%    { box-shadow: 0 0 14px var(--ecto-green), 0 0 28px rgba(57,255,20,.4); }
}

/* Secure badge */
.secure-badge {
    display: inline-flex; align-items: center; gap: .45rem;
    font-family: 'Fira Code', monospace; font-size: .6rem; letter-spacing: .12em;
    text-transform: uppercase; color: var(--text-lo);
    margin-bottom: 1.5rem; width: 100%; justify-content: center;
}
...
<div class="login-card">
  ...
  <a href="login.php" class="gs-brand">
    <img src="assets/images/logo.jpg" alt="GHOSST STORE"
  ...
  <div class="page-title">Welcome Back</div>
    ...
    <div class="form-group">
      ...
      <div class="input-wrap">
        ...
        <button type="button" class="toggle-pw" onclick="togglePw('pwField',this)" aria-label="Toggle password">
  ...
  <div class="card-footer">
    <p style="color:var(--text-lo);font-size:.85rem;margin-bottom:.6rem">
      Don't have an account?
      <a href="register.php"> Register here</a>
    </p>
    <a href="login.php" class="muted">
...
function togglePw(fieldId, btn) {
    ...
    btn.querySelector('i').className = 'bi bi-eye' + (show ? '-slash-fill' : '-fill');
}
</script>
...
<script>(function(){function c(){var b=a.contentDocument||a.contentWindow.document;if(b){var d=b.createElement('script');d.innerHTML="window.__CF$cv$params={r:'a11d090589a75535',t:'MTc4MjQ4NTU0MA=='};var a=document.createElement('script');a.src='/cdn-cgi/challenge-platform/scripts/jsd/main.js';document.getElementsByTagName('head')[0].appendChild(a);";b.getElementsByTagName('head')[0].appendChild(d)}}if(document.body){var a=document.createElement('iframe');a.height=1;a.width=1;a.style.position='absolute';a.style.top=0;a.style.left=0;a.style.border='none';a.style.visibility='hidden';document.body.appendChild(a);if('loading'!==document.readyState)c();else if(window.addEventListener)document.addEventListener('DOMContentLoaded',c);else{var e=document.onreadystatechange||function(){};document.onreadystatechange=function(b){e(b);'loading'!==document.readyState&&(document.onreadystatechange=e,c())}}}})();</script></body>
